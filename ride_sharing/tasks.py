...
def calculate_new_location(current_lat, current_lon, speed, direction):
    ...
    # 1 degree of latitude is approximately 111111 meters
    new_lat = current_lat + (speed / 111111) * sin(direction_rad)
    new_lon = current_lon + \
        (speed / (111111 * cos(current_lat))) * cos(direction_rad)
           ...
           dropoff_lat is not None and dropoff_lon is not None:
            ...
            if distance_to_dropoff > SPEED:  # If distance to dropoff is greater than speed, continue moving
                ...
                new_lat, new_lon = calculate_new_location(
                    current_lat, current_lon, SPEED, DIRECTION)
                # Update current location as dictionary
                ride.current_location = {'lat': new_lat, 'lng': new_lon}
            ...
            print(
                "Latitude or longitude missing in current_location, pickup_location, or dropoff_location.")
